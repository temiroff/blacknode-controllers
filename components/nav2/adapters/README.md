...
Transport adapters for the `nav2` component of `blacknode-controllers`.
...
Declare it in `blacknode-package.toml`:
    ...
    [components.nav2.adapters.ros2]
    description = "ROS 2 adapter for nav2."
    default = false
    capabilities = ["adapter.nav2.ros2"]
    nodes = ["components/nav2/adapters/ros2/nodes"]
